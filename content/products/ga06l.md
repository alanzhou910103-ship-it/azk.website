---
title: "GA06L"
description: "GA06L Charging Grip with Back Paddles for Switch 2 — pass-through USB-C charging, 3 interchangeable back paddle sets, multi-mode compatible. Pro-level control without pausing. Wholesale from int-rock."
tagline: "Joy-Con 2 Charging Grip + Back Paddles"
card_image: "images/ga06l/hero.jpg"
layout: "product"
date: 2026-06-23

hero:
  image: "images/ga06l/hero.jpg"
  badge: "New"

views:
  title: "Product Views"
  images:
    - image: "images/ga06l/hero.jpg"
    - image: "images/ga06l/main.jpg"
    - image: "images/ga06l/detail3.jpg"

highlights:
  - icon: "fas fa-puzzle-piece"
    title: "3 Sets of Replaceable Back Paddles"
    description: "Includes 3 different back button styles — short, long, and hybrid. Swap them tool-free in seconds to match your playstyle for FPS, racing, RPG, or casual gaming."
  - icon: "fas fa-bolt"
    title: "Play While Charging"
    description: "Built-in pass-through USB-C charging port keeps your Joy-Cons powered during intense gaming sessions. No more mid-game battery anxiety."
  - icon: "fas fa-random"
    title: "Mix & Match Back Buttons"
    description: "Not locked to one style. Combine different paddle types on the same grip to customize your control layout exactly how you want."
  - icon: "fas fa-tv"
    title: "Multi-Mode Compatibility"
    description: "Works seamlessly across handheld, tabletop, and TV modes. One grip for every way you play."
  - icon: "fas fa-hand-rock"
    title: "Ergonomic Comfort Grip"
    description: "Contoured handle design reduces hand fatigue during long sessions. Textured surface for a secure, non-slip hold."
  - icon: "fas fa-tools"
    title: "Tool-Free Detachable Design"
    description: "Both the grip handle and back paddles are fully detachable. Switch configurations in seconds — compact for travel storage."

sections:
  - title: "Upgrade Your Gaming with Pro-Level Control"
    image: "images/ga06l/main.jpg"
    image_side: "right"
    content: "The GA06L charging grip is designed for Switch 2 Joy-Con controllers, featuring 3 interchangeable back paddle sets and pass-through charging so you never have to pause the action.\n\nPrecision-molded rails hold Joy-Cons firmly in place with zero wobble, while responsive trigger pass-through ensures no input lag during competitive play."

  - title: "3 Back Paddle Sets for Every Playstyle"
    image: "images/ga06l/detail3.jpg"
    image_side: "left"
    content: "Why settle for one back button layout? The GA06L ships with three complete paddle sets — short, long, and hybrid — so you can switch configurations to match whatever game you're playing. FPS? Go with short triggers. Racing? Use long paddles for precise throttle control.\n\nMix and match different paddle types on the same grip. Unlike competing products that lock you into one configuration, the GA06L gives you complete freedom over your control layout."

  - title: "Charge While You Play"
    image: "images/ga06l/detail1.jpg"
    image_side: "right"
    content: "The built-in USB-C pass-through charging port means your Joy-Cons stay powered while you game. No interruptions, no swapping controllers mid-match, no battery anxiety. Just plug in and keep playing.\n\nWorks in TV mode and tabletop mode — the charging grip becomes your command center for marathon gaming sessions."

  - title: "Tool-Free Customization"
    image: "images/ga06l/detail2.jpg"
    image_side: "left"
    content: "Everything on the GA06L snaps on and off without tools. Swap back paddle sets in seconds. Remove the grip handle for compact storage when traveling. Reconfigure between games without touching a single screw.\n\nThe textured, ergonomic surface provides a secure non-slip hold even during the most intense moments, while the contoured shape significantly reduces hand fatigue over long play sessions."

specs:
  - label: "Compatibility"
    value: "Nintendo Switch 2 Joy-Con controllers"
  - label: "Back Paddles"
    value: "3 interchangeable sets (Short / Long / Hybrid)"
  - label: "Charging"
    value: "USB-C pass-through charging port"
  - label: "Modes"
    value: "Handheld / Tabletop / TV (docked)"
  - label: "Installation"
    value: "Tool-free snap-on design, fully detachable"
  - label: "Surface"
    value: "Ergonomic contour with anti-slip texture"
  - label: "Secure Fit"
    value: "Precision-molded rails, zero wobble"
  - label: "Package Contents"
    value: "1× Charging Grip, 3× Back Paddle Sets (Short/Long/Hybrid), 1× USB-C Cable, 1× User Manual"

cta:
  title: "Want GA06L in your lineup?"
  button_label: "Contact Us for Wholesale"
---
